#ifndef __LOGGER_UTILITY_HPP__
#define __LOGGER_UTILITY_HPP__
...
{
    inline std::string bumpLogFileName(const std::string& _filePath, const std::string& _fileName)
    {
        std::string fullName;
        int counter = 0;
        while(true)
        {
            ++counter;
            fullName = _filePath + "/" + _fileName + "_" + std::to_string(counter) + ".log";
            std::ifstream fil(fullName);
            if(!fil.good())
            {
                fil.close();
                return fullName;
    ...
    const std::unordered_map<log_level, std::string, enum_hasher> colored
            {
                    {log_level::FATAL, " \x1b[35;1m[FATAL]\x1b[0m "},
    ...
    class logger {
    public:
        const static size_t MAX_LOG_MSG_SIZE = 256;
        const static size_t LOG_DATE_AND_SEVERITY = 64;
    ...
    public:
        ...
        virtual void log(const std::string &message, const log_level level) {
            if (level < LOG_LEVEL_CUTOFF)
                return;
            std::string output;
            output.reserve(message.length() + LOG_DATE_AND_SEVERITY);
    ...
    public:
        ...
        file_logger(const logging_config_t &config) : logger(config) {
            //grab the file path
            auto name = config.find("file_path");
            if (name == config.end())
                throw std::runtime_error("No output file path provided to file path logger");

            std::time_t now = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
            std::string date(12, '\0');
            std::strftime(&date[0], date.size(), "%Y-%m-%d", std::localtime(&now));

            std::string file_path = name->second /*+ "/" + date.c_str()*/;
            mkdir(file_path.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
            //grab the file name
            name = config.find("file_name");
            if (name == config.end())
                throw std::runtime_error("No output file provided to file logger");
            file_name = bumpLogFileName(file_path, name->second);
    ...
    {
        std::string envKey = "LOG_TO_CONSOLE";
        char* tmp = getenv(envKey.c_str());
        bool logToConsole = (tmp == NULL ? false : !strcmp(tmp, "true"));
        if(logToConsole)
        {
            logging_config_t newConfig = config;
            newConfig["type"] = "std_out";
            get_logger(newConfig);
        }
        else
        {
            get_logger(config);
    ...
    inline void writeLog(log_level _level, const char *fmt, ...) {
        va_list args;
        va_start(args, fmt);
        std::string buf(logger::MAX_LOG_MSG_SIZE, '\0');
        vsprintf(const_cast<char *>(buf.c_str()), fmt, args);
        va_end(args);
        buf[logger::MAX_LOG_MSG_SIZE - 1] = '\0';
        buf.resize(buf.find('\0'));
        get_logger().log(buf, _level);
    }
    inline void appendLogToFile(const char* _filePath, const char* _fileName, log_level _level, const char *fmt, ...)
    {
        std::string output;
        va_list args;
        va_start(args, fmt);
        ...
        buf[logger::MAX_LOG_MSG_SIZE - 1] = '\0';
        buf.resize(buf.find('\0'));
        output.reserve(buf.length() + logger::LOG_DATE_AND_SEVERITY);
        output.append(timestamp());
        output.append(uncolored.find(_level)->second);
        output.append(buf);
        output.push_back('\n');
        std::string fileFullName = std::string(_filePath) + std::string("/") + std::string(_fileName);
        std::ofstream file;
        try
        {
            file.open(fileFullName, std::ofstream::out | std::ofstream::app);
            file << output;
            file.flush();
            file.close();
        }
...
#define BWR_LOG_FATAL loggerUtility::log_level::FATAL
#define BWR_LOG_ERROR loggerUtility::log_level::ERROR
#define BWR_LOG_WARN loggerUtility::log_level::WARN
#define BWR_LOG_INFO loggerUtility::log_level::INFO
#define BWR_LOG_DEBUG loggerUtility::log_level::DEBUG
#define BWR_LOG_TRACE loggerUtility::log_level::TRACE

#endif // __LOGGER_UTILITY_HPP__
